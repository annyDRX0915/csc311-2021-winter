...
def logistic_predict(weights, data):
    ...
    #####################################################################
    z = np.concatenate([data, np.ones(shape=(data.shape[0], 1))], axis=1)
    z = (z @ weights)
    y = 1 / (1 + np.exp(-z))
    #####################################################################
    #                       END OF YOUR CODE                            #
...
def evaluate(targets, y):
    ...
    frac_correct = np.count_nonzero(np.round(y) == targets) / y.shape[0]
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def logistic(weights, data, targets, hyperparameters):
    ...
    df = (y.reshape(-1, 1) - targets).T @ np.concatenate([data, np.ones(shape=(data.shape[0], 1))], axis=1) / data.shape[0]
    df = df.reshape(-1, 1)
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
