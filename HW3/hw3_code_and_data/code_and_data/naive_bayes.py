...
def train_mle_estimator(train_images, train_labels):
    ...
    label_sum = np.unique(np.argmax(train_labels, axis=1), return_counts=True)[1]
    theta_mle = train_images.T @ train_labels / label_sum
    pi_mle = label_sum / len(train_labels)
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    label_sum = np.unique(np.argmax(train_labels, axis=1), return_counts=True)[1]
    theta_map = (train_images.T @ train_labels + 2) / (label_sum + 4)
    pi_map = label_sum / len(train_labels)
...
def log_likelihood(images, theta, pi):
    ...
    # YOU NEED TO WRITE THIS PART
    summation_term = images @ np.log(theta) + (1 - images) @ np.log(1 - theta)
    last_term = np.log(np.exp(summation_term) @ pi)
    log_like = np.log(pi).reshape(1, -1) + summation_term - last_term.reshape(-1, 1)
    return log_like
...
def accuracy(log_like, labels):
    """ Inputs: matr    ix of log likelihoods and 1-of-K labels
    Returns the accuracy based on predictions from log likelihood values"""

    # YOU NEED TO WRITE THIS PART
    acc = np.count_nonzero(np.argmax(labels, axis=1) == predict(log_like))
    acc = acc/len(labels)
    return acc
